...
1. **Создание:** Пользователь находит специальное сообщение в канале поддержки, нажимает кнопку "Создать тикет" и заполняет модальное окно, при необходимости прикрепляя файлы.
...
4. **Маршрутизация (опционально):** Модератор может передать тикет в профильный отдел (например, "Техническая поддержка"), где им займутся специалисты.
5. **Архивация:** После закрытия тикета бот может автоматически удалить канал через заданное время. Пользователю в ЛС отправляется транскрипция (история переписки).
